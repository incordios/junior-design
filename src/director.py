...
#HOST = (
    #get_ipv4()
#)  # Standard loopback interface address (localhost). When in production, it should match the assigned IP Addr.
            ...
            if key.data is None:
                ...
                try:
                    ...
                    if jsonMSG.get("message") == "Register":
                        ...
                        print(
                            "There are now ",
                            len(registered_robot_map.keys()),
                            " robots registered",
                        )
                        for robot in registered_robot_map.keys():
                            print(
                                f"\tRobot Name: {robot}\tIPv4: {registered_robot_map[robot][0]} "
                            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "csv_file_path", type=str, help="A required integer positional argument"
    )
    ...
    Registration = multiprocessing.Process(
        target=registration,
        args=(
            HOST,
            PORT,
            Registered_Robots_Map,
        ),
    )
